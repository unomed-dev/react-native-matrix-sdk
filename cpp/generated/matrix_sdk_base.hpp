...
protected:
  ...
  jsi::Value cpp_uniffi_internal_fn_func_ffi__arraybuffer_to_string(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
  jsi::Value cpp_ffi_matrix_sdk_base_uniffi_contract_version(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
...
public:
   ...
   *
   * React Native must call `NativeMatrixSdkBase.registerModule(rt,
   ...
   * be the case.
   *
   * Clients should call `NativeMatrixSdkBase.unregisterModule(rt)` after final
   * use where possible.
   */
