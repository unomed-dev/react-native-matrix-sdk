...
class NativeMatrixSdkBase : public jsi::HostObject {
  private:
  ...
  protected:
    ...
    jsi::Value cpp_ffi_matrix_sdk_base_uniffi_contract_version(jsi::Runtime& rt, const jsi::Value& thisVal, const jsi::Value* args, size_t count);

  public:
    NativeMatrixSdkBase(jsi::Runtime &rt, std::shared_ptr<uniffi_runtime::UniffiCallInvoker> callInvoker);
    virtual ~NativeMatrixSdkBase();

    /**
     * The entry point into the crate.
     *
     * React Native must call `NativeMatrixSdkBase.registerModule(rt, callInvoker)` before using
     ...
     *
     * Clients should call `NativeMatrixSdkBase.unregisterModule(rt)` after final use where possible.
     */
