...
protected:
  ...
  jsi::Value cpp_uniffi_internal_fn_func_ffi__arraybuffer_to_string(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
  jsi::Value cpp_ffi_matrix_sdk_common_uniffi_contract_version(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);

public:
  NativeMatrixSdkCommon(
      jsi::Runtime &rt,
      std::shared_ptr<uniffi_runtime::UniffiCallInvoker> callInvoker);
   ...
   *
   * React Native must call `NativeMatrixSdkCommon.registerModule(rt,
   ...
   * be the case.
   *
   * Clients should call `NativeMatrixSdkCommon.unregisterModule(rt)` after
