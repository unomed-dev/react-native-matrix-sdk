...
class NativeMatrixSdkUi : public jsi::HostObject {
  private:
  ...
  protected:
    ...
    jsi::Value cpp_ffi_matrix_sdk_ui_uniffi_contract_version(jsi::Runtime& rt, const jsi::Value& thisVal, const jsi::Value* args, size_t count);

  public:
    NativeMatrixSdkUi(jsi::Runtime &rt, std::shared_ptr<uniffi_runtime::UniffiCallInvoker> callInvoker);
    virtual ~NativeMatrixSdkUi();

    /**
     * The entry point into the crate.
     *
     * React Native must call `NativeMatrixSdkUi.registerModule(rt, callInvoker)` before using
     ...
     *
     * Clients should call `NativeMatrixSdkUi.unregisterModule(rt)` after final use where possible.
     */
