...
class DispatchingRoomListEntriesListener: RoomListEntriesListener {
  ...
  override fun onUpdate(roomEntriesUpdate: List<RoomListEntriesUpdate>) {
    eventEmitter.emit(eventName, Arguments.createMap().apply {
      putArray("roomEntriesUpdate", Arguments.createArray().apply {
        roomEntriesUpdate.forEach { pushMap(roomListEntriesUpdateToMap(it)) }
      })
    })
  }
}
