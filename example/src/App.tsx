...
const startClient = async (
  ...
  onRoomListEntriesUpdate: (roomEntriesUpdate: RoomListEntriesUpdate[]) => void
) => {
  if (!client) {
    return;
  }
  ...
  const logInWithSso = React.useCallback(async () => {
    ...
    try {
      ...
      await startClient(
        ...
        async (roomEntriesUpdate) => {
          console.log(
            `RoomListEntriesListener fired: ${roomEntriesUpdate.map((u) => u.type).join(', ')}`
          );
